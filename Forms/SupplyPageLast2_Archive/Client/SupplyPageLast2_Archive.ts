export default class SupplyPageLast2_Archive extends Form.SYNERGY {

	async ctxSupplyMenu_OnShowing(args: Controls.EventArgs.IContextMenuOnShowingEventArgs) {
	...
	async SupplyPageLast2_Archive_OnLoad(args: Controls.EventArgs.ILoadEventArgs) {
        await this.applyChanges().then(() => this.FILTERPARAMS.value = "1");
	}

	async ctxSupplyMenu_OnItemClick(args: Controls.EventArgs.IContextMenuItemClickEventArgs) {
        if (args.baseArgs.data.key === "Edit") {
            ...
            this.deleteItem(args, "DeletePartnerSupply", this.SupplyPageLast2_Archive_Grid);
        }
            ...
            return await this.showConfirm(title, message).then(() => {
                this.fetch.post("DataSource/UpdateSupplyStatus", { DocumentId: args.targetArgs.row.data.DOCUMENTID }).then(result => {
                    this.SupplyPageLast2_Archive_Grid.reload(true);
                    this.applyChanges();
                });
            }).catch(() => {
                args.baseArgs.cancel = true;
        ...
        else if (args.baseArgs.data.key === "View") {
            this.openForm({
                projectName: "SupplierIdentification",
                typeName: "OpenFormArgs",
                documentId: args.targetArgs.row.data.DOCUMENTID,
	...
	async SupplyPageLast2_Archive_OnMessageReceived(args: Controls.EventArgs.IMessageReceivedEventArgs) {
        this.SupplyPageLast2_Archive_Grid.reload(true);
	}

	async SupplyPageLast2_Archive_OnToolbarButtonClicked(args: Controls.EventArgs.IToolbarButtonClickEventArgs) {
        if(args.name === "New")
        {
            this.createForm(
                {
